...
if type exa > /dev/null 2>&1; then
  ...
  alias  lll="${BASH_ALIASES[ll]} --classify"
  alias llll="${BASH_ALIASES[lll]} --extendeda"
  alias llrt="${BASH_ALIASES[ll]} --sort=mod"
else
  # /bin/ls options:
  #
...
function demo_ls_colors () {
    ...
    local FILE03=/var/run/log #$DIR/socket
    local FILE04=4-pipe
    local FILE05=5-executable
    # no block devices on FreeBSD
    ...
    local FILEGRP=$(groups | awk '{ print $1 }')
    #
    # make examples
    #
    mkdir -p $DIR; cd $DIR
    ...
    touch    $FILE08 ;                          chmod o+sx   $FILE08
